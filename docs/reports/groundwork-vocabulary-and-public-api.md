...
- The route-native `Groundwork.Core.SchemaEvolution` model is the canonical desired-state diff, durable-state, locking, and safe-application contract introduced by #44. `Groundwork.Materialization` remains a compatibility planner/executor surface until providers move to executable routes in #46–#48. The unused duplicate `Groundwork.Core.Materialization` records and the independent imperative migration pipeline still require convergence.
...
| Current public API | Runtime role verified in source | Assessment |
|---|---|---|
| [`Groundwork.Materialization.MaterializationPlan`](../../src/Groundwork/Materialization/MaterializationPlan.cs) | Compatibility provider plan used by the current provider materializers and document-store factories. Its additive-index rebuild is now an explicit canonical-JSON backfill operation. | Preserve until #46–#48 move providers to route-native schema diffs; do not grow it into a competing desired-state ledger. |
...
`Groundwork.Core.SchemaEvolution` owns the route-native desired-state diff and safe-application contracts. `Groundwork.Materialization` remains the compatibility provider plan until #46–#48; it schedules the Core canonical-JSON-backfill operation directly. Reconcile the remaining `Groundwork.Core.Materialization` records and compatibility contracts in the planned vocabulary cleanup rather than maintaining parallel authorities indefinitely.
