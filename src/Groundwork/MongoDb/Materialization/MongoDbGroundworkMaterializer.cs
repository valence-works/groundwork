...
public sealed class MongoDbGroundworkMaterializer(IMongoDatabase database, Action<string>? recordAdvisory = null)
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await EnsureMutationIndexesAsync(model, cancellationToken);
        return result;
    }

    private async Task EnsureMutationIndexesAsync(
        MongoDbPhysicalStorageModel model,
        CancellationToken cancellationToken)
    {
        foreach (var route in model.Routes)
        {
            ...
            foreach (var query in Documents.MongoDbPhysicalMutationStorage.MutationQueries(storage))
            {
                await EnsureMutationIndexAsync(
                    database.GetCollection<BsonDocument>(route.PrimaryStorage.Name.Identifier),
                    route,
                    storage,
                    query,
                ...
                {
                    await EnsureMutationIndexAsync(
                        database.GetCollection<BsonDocument>(route.LinkedIndexStorage.Name.Identifier),
                        route,
                        storage,
                        query,
    ...
    private static async Task EnsureMutationIndexAsync(
        ...
        CancellationToken cancellationToken)
    {
        var discriminator = target == ExecutableStorageObjectRole.PrimaryStorage
            ? route.Envelope.DocumentKind.Identifier
            : route.LinkedRelationship!.DocumentKind.Identifier;
        var scope = target == ExecutableStorageObjectRole.PrimaryStorage
        ...
        var keys = Builders<BsonDocument>.IndexKeys.Combine(
            new[]
            {
                Builders<BsonDocument>.IndexKeys.Ascending(discriminator),
                Builders<BsonDocument>.IndexKeys.Ascending(scope)
            }.Concat(Documents.MongoDbPhysicalMutationStorage.IndexPaths(storage, query)
                .Select(path => Builders<BsonDocument>.IndexKeys.Ascending(
                    target == ExecutableStorageObjectRole.PrimaryStorage
                        ? Documents.MongoDbPhysicalMutationStorage.PrimaryField(route, path)
                        : Documents.MongoDbPhysicalMutationStorage.LinkedField(route, path)))));
        var index = new CreateIndexModel<BsonDocument>(
            keys,
            new CreateIndexOptions<BsonDocument>
            {
                Name = Documents.MongoDbPhysicalMutationStorage.IndexName(route, query, target),
            ...
            exception.Code is IndexOptionsConflictErrorCode or IndexKeySpecsConflictErrorCode)
        {
            throw new InvalidOperationException(
                $"MongoDB bounded-mutation index '{index.Options.Name}' on collection " +
                $"'{collection.CollectionNamespace.CollectionName}' conflicts with the compiled store model.",
                exception);
        }
    }
