# Feature Specification: Groundwork Host Integration Bridge

**Feature Branch**: `codex/groundwork-host-integration-bridge`

**Created**: 2026-06-10

**Status**: Draft

**Input**: User description: "Groundwork host integration bridge and first opt-in module validation. Add a `Groundwork.Hosting` integration package that can register Groundwork manifests and providers, materialize startup plans through application host startup tasks, expose diagnostics for registered manifests/providers/materialization status, and validate a low-risk Secrets-like module manifest through tests without adding host-specific concepts to Groundwork packages."

## User Scenarios & Testing

### User Story 1 - Register Groundwork In An Application Host (Priority: P1)

An application host integrator can opt into Groundwork persistence by enabling a host integration bridge package and registering one or more Groundwork manifests.

**Why this priority**: Without a host integration bridge, Groundwork remains a standalone library and cannot validate the integration boundary promised by G3.

**Independent Test**: Configure the bridge in an `IServiceCollection`, register a manifest, build the provider, and verify bridge options plus diagnostics expose the manifest without adding host-specific references to Groundwork packages.
...
2. **Given** generic Groundwork packages, **When** bridge services are added, **Then** Groundwork packages still do not reference host-specific projects.

---

### User Story 2 - Materialize Groundwork At Application Host Startup (Priority: P2)

An application host can register provider adapters and let an application host startup task materialize configured Groundwork manifests.

**Why this priority**: Startup materialization proves Groundwork can participate in application host composition without EF provider-specific migrations for the Groundwork-backed path.
...
An application host operator or test can inspect what manifests, providers, and materialization attempts are currently known to the bridge.
...
- **FR-001**: Add a `Groundwork.Hosting` project under the host integration area.
- **FR-002**: The bridge MUST register Groundwork manifests through application host service configuration.
- **FR-003**: The bridge MUST register provider adapters without referencing provider-specific packages from the bridge project.
- **FR-004**: The bridge MUST expose an application host startup task that materializes configured manifests when enabled.
...
- **FR-008**: Add a Secrets-like application-host manifest factory in the bridge or tests as the first low-risk validation target.
- **FR-009**: Tests MUST prove the Secrets-like manifest can be materialized by SQLite through the bridge.
- **FR-010**: Tests MUST prove bridge registration does not add host-specific references to Groundwork packages.
- **FR-011**: The bridge MUST be opt-in and must not replace existing EF persistence paths.

### Key Entities

- **Groundwork Bridge Options**: application host-side configuration containing manifests and materialization behavior.
- **Groundwork Provider Adapter**: application host-side adapter that names a Groundwork provider and can materialize a manifest.
- **Groundwork Startup Task**: application host startup task that coordinates configured manifests and provider adapters.
- **Groundwork Diagnostics Snapshot**: Read model listing manifests, providers, and materialization outcomes.
- **Secrets-like Manifest**: Low-risk host integration validation manifest stored as Groundwork documents and indexes.
...
- **SC-004**: Architecture tests verify Groundwork projects remain free of host-specific dependencies after the bridge is added.

## Assumptions

- The bridge package may reference application host composition/task abstractions and Groundwork contracts.
